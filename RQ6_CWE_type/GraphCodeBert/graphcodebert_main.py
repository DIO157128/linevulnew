...
def test(args, model, tokenizer, test_dataset):
    ...
    df = pd.read_csv('../../data/big-vul_dataset/cwe_test.csv')
    funcs = df["processed_func"].tolist()
    df = pd.DataFrame()
    df['processed_func'] = funcs
    df["CWE ID Label"] = y_trues
    df['raw_prediction'] = y_preds
    df.to_csv('../results/graphcodebert/raw_preds.csv')
...
def main():
    ...
    model = RobertaModel.from_pretrained("microsoft/graphcodebert-base", config=config, ignore_mismatched_sizes=True)
