...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    f = open("../results/graphcodebert/func_res_{}.txt".format(args.fine_tune_factor), "a")
    for key in sorted(result.keys()):
    ...
    if args.do_sorting_by_line_scores:
        ...
        for reasoning_method in all_reasoning_method:
            ...
            with open("../results/graphcodebert/line_res_{}.txt".format(args.fine_tune_factor), "a") as f:
                f.write(to_write)
    ...
    if args.do_local_explanation:
        for reasoning_method in all_reasoning_method:
            ...
            with open(f"../results/ifa_records/graphcodebert_ifa_{reasoning_method}_{args.fine_tune_factor}.txt", "w+") as f:
            ...
            # output results
            with open("../results/graphcodebert/local_explanation_{}.txt".format(args.fine_tune_factor), "a") as f:
...
def write_raw_preds_csv(args, y_preds):
    ...
    df.to_csv("../results/graphcodebert/raw_preds_{}.csv".format(args.fine_tune_factor), index=False)
...
def line_level_localization_tp(flaw_lines: str, tokenizer, model, mini_batch, original_func: str, args, top_k_loc: list, top_k_constant: list, reasoning_method: str, index: int, write_invalid_data: bool):
    ...
    if do_explanation:
        ...
        if write_invalid_data:
            with open("../results/invalid_data/graphcodebert_invalid_line_lev_data.txt", "a") as f:
