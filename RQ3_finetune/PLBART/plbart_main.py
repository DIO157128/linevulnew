...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    f = open("../results/plbart/func_res_{}.txt".format(args.fine_tune_factor), "a")
    for key in sorted(result.keys()):
    ...
    if args.do_sorting_by_line_scores:
        ...
        for reasoning_method in all_reasoning_method:
            ...
            with open("../results/plbart/line_res_{}.txt".format(args.fine_tune_factor), "a") as f:
                f.write(to_write)
    ...
    if args.do_local_explanation:
        for reasoning_method in all_reasoning_method:
            ...
            with open(f"../results/ifa_records/plbart_ifa_{reasoning_method}_{args.fine_tune_factor}.txt", "w+") as f:
            ...
            line_level_results = [{f"plbart_{reasoning_method}_top20%_recall":
                                ...
                                f"plbart_{reasoning_method}_recall@topk%loc_auc":
...
def write_raw_preds_csv(args, y_preds):
    ...
    df.to_csv("../results/plbart/raw_preds_{}.csv".format(args.fine_tune_factor), index=False)
...
def line_level_localization_tp(flaw_lines: str, tokenizer, model, mini_batch, original_func: str, args, top_k_loc: list, top_k_constant: list, reasoning_method: str, index: int, write_invalid_data: bool):
    ...
    if do_explanation:
        if reasoning_method == "attention":
            ...
            lig = LayerIntegratedGradients(lig_forward, model.module.plbart.model.shared)
            ...
            # send data to device
            input_ids = input_ids.to(args.device)
            input_embed = model.module.plbart.model.shared(input_ids).to(args.device)
